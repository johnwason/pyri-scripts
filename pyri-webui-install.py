...
def install_indirect_packages(pruned_install, webui_wheels_dir):
    with tempfile.TemporaryDirectory() as tmpdir1:
        tmpdir = Path(tmpdir1)
        ...
        subprocess.check_call([sys.executable, "-mpip", "download", f"-r{requirements_fname}", f"--dest={webui_wheels_dir}", "--no-deps"])
...
def install_direct_packages(pruned_install, webui_wheels_dir):
    for p in pruned_install:
        if p["is_direct"]:
            remove_existing_package(p["metadata"]["name"], webui_wheels_dir)
...
def main():
    pip_passthrough_args = sys.argv[1:]
    no_deps = False
    ...
    install_indirect_packages(pruned_install, wheels_dir)
    install_direct_packages(pruned_install, wheels_dir)
